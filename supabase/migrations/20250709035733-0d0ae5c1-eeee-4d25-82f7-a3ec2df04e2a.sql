...
-- Verificar se as tabelas de mensagens já existem e adicionar campos necessários
-- Adicionar campo delivery_code se não existir
ALTER TABLE public.message_history 
ADD COLUMN IF NOT EXISTS delivery_code TEXT UNIQUE DEFAULT CONCAT('MSG-', EXTRACT(EPOCH FROM now())::bigint, '-', SUBSTRING(gen_random_uuid()::text, 1, 8));

-- Criar tabela de destinatários de mensagem se não existir
...
-- Política RLS para message_recipients
ALTER TABLE public.message_recipients ENABLE ROW LEVEL SECURITY;
CREATE POLICY IF NOT EXISTS "Allow all access to message_recipients" 
